...
class OfferList extends React.Component {
  ...
  loadOffers = async () => {
    ...
    try {
      let res = await fetch(
        `${API}offers?offset=${this.state.page >= 0 ? this.state.page : 0}`,
        {
          method: "get",
          headers: {
            Accept: "*/*",
      ...
      this.setState(
        {
          loading: false,
          offers: data.offers,
          wasLastItem: data.offers.length === 1,
        },
        () => {
  ...
  deleteOffer = async (docID) => {
    this.setState({ loading: true });
    try {
      let res = await fetch(`${API}offers?id=${docID}`, {
        method: "delete",
        headers: {
          Accept: "*/*",
          Authorization: "Bearer " + this.props.user.token,
